...
/**
 * Bean for reading temperature and humidity values from the connected DHT-22
 * (Or DHT-11) sensor.
 *
 * @author andre
 */
...
public class TempSensorReader {
    ...
    /**
     * Try to read the temperature and humidity of the DHT-22 sensor. If the
     * measured data is faulty, it will trigger the measurement several times
     * until its successful (default = 10 tries).
     *
     * @param pin number of the used input pin for the DHT-22.
     * @return Optional Map with the values, or an empty optional, if the
     * measurement wasnt succesful after the maximum amount of tries.
     * @throws InterruptedException If the sleep commands were throwing
     * Exceptions.
     */
    public Optional<Map<String, Float>> getTempAndHumidity(final int pin) throws InterruptedException {
        ...
        if (!returnValue.isPresent()) {
            logger.warn("Couldnt read values from temperature sensor! Failed attempts: {}", MAX_READ_ATTEMPTS);
        }
